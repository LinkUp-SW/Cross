...
class SearchOrganizationPage extends ConsumerStatefulWidget {
  final String? initialQuery;
  const SearchOrganizationPage({Key? key, this.initialQuery}) : super(key: key);

  @override
  ConsumerState<SearchOrganizationPage> createState() => _SearchOrganizationPageState();
}

class _SearchOrganizationPageState extends ConsumerState<SearchOrganizationPage> {
  ...
  Future<void> _fetchResults(String query) async {
     ...
     try {
        ...
        await notifier.getcompany(query, ref);

        if (mounted) {
          final companyResults = ref.read(companyResultsProvider);
          setState(() {
             _results = companyResults;
             _isLoading = false;
             if (_results.isEmpty) {
        ...
        log("Error fetching organization suggestions: $e");
        if (mounted) {
          setState(() {
             _isLoading = false;
             _results = [];
          ...
          return ListTile(
             leading: CircleAvatar(
                ...
                child: (logo == null || logo.isEmpty)
                    ? Icon(Icons.business, size: 20.sp, color: isDarkMode ? AppColors.darkGrey : AppColors.lightGrey)
                    : null,
             ),
             title: Text(
                name,
